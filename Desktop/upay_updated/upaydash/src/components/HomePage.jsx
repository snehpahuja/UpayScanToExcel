...
import { clearTokens, getAuthUser, isAuthenticated, fetchWithAuth, handleApiResponse } from '../lib/auth';
...
export default function HomePage() {
  ...
  const [filterDocType, setFilterDocType] = useState('All Document Types');
  ...
  async function fetchStats() {
    try {
      ...
      try {
        ...
        statsData = await handleApiResponse(res);
      } catch (err) {
        try {
      ...
      if (statsData && statsData.total_documents !== undefined) {
        setTotalDocuments(statsData.total_documents);
      }
    } catch (err) {
  ...
  async function fetchFilteredStats(city, docType, date) {
    setLoadingStats(true);
    try {
      const params = new URLSearchParams();
      if (city && city !== 'All Cities') params.set('city', city);
      if (docType && docType !== 'All Document Types') params.set('document_type', docType);
      if (date) params.set('date', date);

      const res = await fetchWithAuth(`${API_BASE}/api/stats/filter/?${params.toString()}`);
      const data = await handleApiResponse(res);
      if (data && data.total_documents !== undefined) {
          ...
          <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
            ...
            <div className="mt-12 bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <select
                  className="p-2 border rounded-md bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500"
                  value={filterDocType}
                  onChange={(e) => {
                    const v = e.target.value;
                    setFilterDocType(v);
                    fetchFilteredStats(filterCity, v, filterDate);
                  }}
                >
                  <option>All Document Types</option>
                  <option>Student Records</option>
                  <option>Attendance</option>
                  <option>Survey Forms</option>
                </select>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const v = e.target.value;
                    setFilterDate(v);
                    fetchFilteredStats(filterCity, filterDocType, v);
                  }}
                  className="p-2 border rounded-md bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500"
                />
                ...
                <select
                  ...
                  onChange={(e) => {
                    const v = e.target.value;
                    setFilterCity(v);
                    fetchFilteredStats(v, filterDocType, filterDate);
                  }}
                >
                ...
                {loadingStats ? (
                  ...
                  <>
                    <p className="text-4xl font-bold text-indigo-900 mt-2">
                      {totalDocuments !== null ? totalDocuments.toLocaleString() : '—'}
                    </p>
                    {totalDocuments === null && (
                      <p className="text-xs text-gray-500 mt-1">API endpoint not available</p>
                    )}
                  </>
                )}
              </div>
